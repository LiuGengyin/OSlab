...
void interrupt_helper(regs_context_t *regs, uint64_t stval, uint64_t scause)
{
    ...
    if(!(scause >> 63)) exc_table[scause](regs, stval, regs->regs[17]);//syscall
    else irq_table[scause & (~0x80000000)](regs, stval, scause);//handle interrupt
}
