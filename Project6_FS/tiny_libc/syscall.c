                           ...
                           long arg3, long arg4)
{
    ...
    return res;
}//depart from here(ecall) and finally back to here(sret).
...
int sys_ps(int startline)
{
    /* TODO: [p3-task1] call invoke_syscall to implement sys_ps */
    return invoke_syscall(SYSCALL_PS, startline, IGNORE, IGNORE, IGNORE, IGNORE);
}
...
int sys_cd(char *path, int p)
{
    // TODO [P6-task1]: Implement sys_cd
    return invoke_syscall(SYSCALL_FS_CD, path, p, IGNORE, IGNORE, IGNORE);  // sys_cd succeeds
}
...
int sys_lseek(int fd, int offset)
{
    ...
    return invoke_syscall(SYSCALL_FS_LSEEK, fd, offset, IGNORE, IGNORE, IGNORE);  // the resulting offset location from the beginning of the file
}
