...
static void create_image(int nfiles, char *files[])
{
    ...
    for(int i = 0; i < 4; i++){
        ...
        printf("taskname: %s\n", taskinfo[i].name);
        printf("size: %d\n", taskinfo[i].size_of_task);
        printf("start_from_this_sector: %d\n", taskinfo[i].start_from_this_sector);
        printf("offset_from_this_sector: %d\n", taskinfo[i].offset_from_this_sector);
    }
}
