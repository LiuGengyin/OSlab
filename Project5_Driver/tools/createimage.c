...
static void write_img_info(int nbytes_kern, task_info_t *taskinfo, short tasknum, FILE * img)
{
    ...
    short nowblocks = NBYTES2SEC(phyaddrbuf); //image文件所用扇区数，在这之后的所有扇区可用于存储换出的页。写在0x1f6~0x1f7 （编号为nowblocks的扇区已经被占用了）
    
    //继续写，loader里取出这两个数据，装载对应的两个扇区，并把正确的taskinfo写在 处。再看后面有无bug。
    //低地址写低位
    printf("size of info is: %hd\n", size_of_info);
    printf("start addr is: %d\n", st_addr);
    fseek(img, OS_SIZE_LOC - 6, SEEK_SET);
    fputc((char)(0xff&nowblocks), img); fputc((char)(0xff&(nowblocks>>8)), img);
    fputc((char)(0xff&st_addr), img); fputc((char)(0xff&(st_addr>>8)), img); fputc((char)(0xff&(st_addr>>16)), img); fputc((char)(0xff&(st_addr>>24)), img);
